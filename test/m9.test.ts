...
describe("M9 installation and launch foundation", () => {
  ...
  test("keeps a legacy project config instead of moving user data", () => {
    ...
    writeConfig(defaultConfig(), legacy);
  ...
  test("does not treat an unrelated project config as legacy application data", () => {
    const directory = mkdtempSync(join(tmpdir(), "bcts-m9-unrelated-"));
    directories.push(directory);
    writeFileSync(join(directory, "config.json"), "{}", "utf8");
    const environment = { BCTS_DATA_DIR: join(directory, "application-data") };
    expect(preferredConfigPath(environment, directory)).toBe(
