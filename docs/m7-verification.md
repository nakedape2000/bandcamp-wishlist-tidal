...
- repeating the action with unchanged state does not create duplicate additions;
...
M7 is accepted only when every section above has a recorded result and the
fixture, mocked-apply, and real-account read-only checks agree on the same state.
